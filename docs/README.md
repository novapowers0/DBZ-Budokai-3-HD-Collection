...
- **Estado**: **v1.2.9 en el árbol** (pendiente de publicar) / **v1.2.8.2 publicada (Latest)** - La v1.2.9 es **diagnóstico que se explica solo**: avisos SIEMPRE activos de fps sostenido, disco lento e **instalación mixta** (las DLL publican su sello de build en `rex/dbz3_build.h`, porque no tienen VERSIONINFO), `vram=`/`lim=` en la línea `perf` y una línea `dbz3: entorno os=... ram=... <versiones>` al arrancar; guardia de VRAM (no se conceden upscales nuevos con el heap local al 92 %) y `tools/copy_sdk_dlls.ps1`. Sobre la v1.2.8.2 (la mejora de texturas deja de hundir los fps: solo nivel 0 en recargas dinámicas), la v1.2.8.1 (volcado de los formatos del HUD/UI + packs RGBA8, issue #11), la v1.2.8 (fix del volcado: la cvar del launcher no llegaba al plugin), la v1.2.7 (packs de texturas estilo PCSX2, D3D12 y Vulkan), la v1.2.6 (mejora de texturas HD pulida, HUD limpio, UX de escala y autorreparación del `dbz3_user.toml`), la v1.2.5 (foco/disco), la v1.2.4 EX, la v1.2.3 y la v1.2.2 EX (auto-detección del ejecutable + modo ISO retail). Juego funcional (D3D12, 60 fps, US+EU). Swap nativo HD↔HD y texturas funcionan (Vía A aproximada). Port completo PS2→HD **aparcado** (§3.4.10). Escalado: FSR1/CAS sí; FSR3/DLSS no viable a corto plazo. El consumo alto es el **supersampling** (escala interna), no las texturas HD.
