...
> x1..x4 y generación de mips.
>
> **v1.2.8.1 — el volcado cubre también los formatos del HUD/UI**: además de los
...
> solo se volcaban los DXT y casi todo el HUD desaparecía del volcado.
> **Reemplazables por un pack**: DXT1/3/5 y **RGBA8** (`k_8_8_8_8`); los de
> 8/16 bits se vuelcan como referencia pero su pack todavía **no** se aplica.
...
3. Al cargar una textura, si su hash está en el pack, se usa la imagen del pack
   (decodificada a RGBA8) a la resolución que tenga el fichero (x1..x4) y se
...
mods/
  MiPack/
    ...
    04037CE1AFA993E9_1024x512_DXT3.png     <- también vale PNG
...
- Nombre: **`<hash:16 hex>_<Ancho>x<Alto>_<sufijo>.dds`** (o `.png`).
  ...
  - `Ancho`/`Alto` = tamaño de **esta** imagen (la del pack).
...
- Formatos admitidos: **DDS** (DXT1/BC1, DXT3/BC2, DXT5/BC3, o 32bpp sin
  ...
  Como el pack se sube siempre como **RGBA8**, lo normal es exportar el PNG (así
...
## 3. Cómo crear un pack (paso a paso)
...
2. Reinicia y juega. Se escriben los DDS + `index.jsonl` (hash, tamaño, formato,
   sufijo DDS, mips) de cada textura única. El nombre del fichero lleva el
...
- **Formatos no soportados**: se omiten y queda **un aviso por formato** en el
  ...
  Así se ve de un vistazo que queda pendiente (hoy: `k_DXN`/normales,
  `k_DXT5A`/alpha, `k_24_8`, 16_16_16_16...).
- **Vídeo/render targets**: una textura cuyo contenido cambia en cada uso (el
  vídeo de la intro) se vuelca como máximo **4 versiones** y luego se deja de
...
- **Límite total**: `dbz3_texture_dump_max` (por defecto 4096, `0` = sin límite)
  en el tab Dev.
...
casan con ningún `#AZT` van a `_unknown\`.
...
> el dato exacto del juego. Si quieres verlas/utilizarlas, añade
...
**el mismo nombre de fichero**; solo cambia el contenido y el tamaño.

### 3.4 Empaquetar

Crea `mods\MiPack\` y guarda ahí las imágenes escaladas **renombradas al
...
si le pasas el volcado, que el hash exista y que el factor sea válido (1..4).
...
- **Formatos reemplazables**: DXT1/DXT3/DXT5 y **RGBA8** (`k_8_8_8_8`). Los
  ...
  **sí** se vuelcan (referencia/edición) pero su pack se **ignora**: su recurso
...
- **Conflictos**: si dos packs definen el mismo hash, gana el primero por orden
  alfabético de carpeta; el runtime lo avisa en el log.
- Los packs **no se aplican en modo disco (ISO)**: requieren la carpeta extraída
  (igual que el resto de mods).
- El factor máximo es **x4**. Un fichero cuyo tamaño no sea múltiplo entero del
...
- `packs cvar = ''` → el launcher no detectó ningún pack (revisa la carpeta).
- `no es multiplo de ...` → el tamaño del fichero no cuadra con el original.
...
- Runtime (SDK): `rexglue-sdk-0.10/src/graphics/dbz3_texture_pack.{h,cpp}`
  (módulo COMÚN a los dos backends: índice, tabla de formatos volcables
  `Dbz3DumpFormatFor`, `Dbz3PackReplaceableFormat`, decodificación DDS/BC) y
...
- Herramientas: `awo_tools/texture_dump_import.py` (DDS→PNG y organización),
