...
static void InvalidFunctionTrap(PPCContext& ctx, uint8_t* /*base*/) {
  // Diagnostic mode (DBZ3_COLLECT_UNREGISTERED): log each unregistered function
  // to a file and continue instead of aborting, so a single boot enumerates all
  // missing indirect-call targets. Normal mode aborts with REX_FATAL.
  ...
  if (getenv("DBZ3_COLLECT_UNREGISTERED")) {
    if (seen.insert(addr).second) {
      if (!collect_file) {
        collect_file = fopen("dbz3_unregistered.txt", "w");
      }
      if (collect_file) {
        fprintf(collect_file, "0x%08X\n", addr);
        fflush(collect_file);
      }
      REXLOG_CRITICAL("UNREGISTERED FUNCTION 0x{:08X}", addr);
    }
    return;
  }
  REX_FATAL("Call to invalid or unregistered function at guest address 0x{:08X}", addr);
}
