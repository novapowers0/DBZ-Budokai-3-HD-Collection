...
// user_language is defined at global scope in xam_user.cpp (before the
// namespace block), so its accessor lives outside rex::kernel::xam.
...
u32 XGetLanguage_entry() {
  // Return the language chosen in the launcher (dbz3_language, propagated to
  // user_language). Defaults to English if unset. The game reads this to pick
  // its UI text language, so the launcher selector controls the in-game text.
  return static_cast<uint32_t>(REXCVAR_GET(user_language));
}
