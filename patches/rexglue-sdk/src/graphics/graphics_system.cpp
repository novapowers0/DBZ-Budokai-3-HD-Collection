  ...
  vsync_worker_thread_ = system::object_ref<system::XHostThread>(
      new system::XHostThread(kernel_state_, 128 * 1024, 0, [this]() {
        ...
        while (vsync_worker_running_) {
          ...
          // The guest game logic MUST always tick at the display rate (60 Hz):
          // with the `vsync` cvar off the interval would collapse to ~1 ms
          // (a 1000 Hz vblank) and the game would run ~16x too fast. Clamp so
          // that even if the cvar is disabled the vblank can never be shorter
          // than one 60 Hz frame (vsync=false becomes a no-op).
