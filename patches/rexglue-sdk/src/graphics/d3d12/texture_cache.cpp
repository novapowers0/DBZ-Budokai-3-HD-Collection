...
#include "../shaders/bytecode/d3d12_5_1/texture_upscale_cs.h"
}  // namespace shaders

// DBZ3 HD Collection: factor de upscale de texturas guest en la cache host.
// 1 = desactivado (comportamiento original). Se aplica en la carga de la
...
struct TextureUpscaleConstants {
  ...
  uint32_t src_offset;
  uint32_t factor;
  uint32_t level;  // nivel de mip que se escribe (0 = base)
};
...
uint32_t D3D12TextureCache::GetTextureUpscaleFactor(const TextureKey& key) const {
  ...
  // (formato/tamano/mips/array + motivo) porque el frontbuffer de 720p se
  // consulta continuamente y consumiria el cupo antes de ver texturas de juego.
  auto reject = [&](int code, const char* why) -> uint32_t {
    static constexpr int kMaxLogged = 28;
    static int logged = 0;
    static uint64_t seen[kMaxLogged] = {};
    const uint64_t sig = (uint64_t(uint32_t(key.format)) << 48) |
                         (uint64_t(code & 0xF) << 44) |
                         (uint64_t(uint32_t(key.dimension) & 0xF) << 40) |
                         (uint64_t(key.mip_max_level & 0xF) << 36) |
                         (uint64_t(key.GetDepthOrArraySize() & 0xF) << 32) |
    ...
    if (logged < kMaxLogged) {
      seen[logged++] = sig;
      REXGPU_INFO(
          "dbz3: upscale skip [{}] fmt={} dim={} mips={} array={} {}x{} scaled_resolve={} "
          "signed_sep={}",
          why, uint32_t(key.format), uint32_t(key.dimension), uint32_t(key.mip_max_level),
          uint32_t(key.GetDepthOrArraySize()), key.GetWidth(), key.GetHeight(),
          key.scaled_resolve ? 1 : 0, key.signed_separate ? 1 : 0);
    }
    return 1;
  };
  ...
  if (key.scaled_resolve) {
    return reject(2, "scaled_resolve");
  }
  if (key.signed_separate) {
    return reject(3, "signed_separate");
  }
  ...
  if (width > (D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION / factor) ||
...
std::unique_ptr<TextureCache::Texture> D3D12TextureCache::CreateTexture(TextureKey key) {
  ...
  desc.Alignment = 0;
  uint32_t upscale_factor = GetTextureUpscaleFactor(key);
  ...
  if (key.scaled_resolve) {
    ...
    // DBZ3: el recurso host se crea a Nx y se rellena con la pasada de upscale.
    desc.Width *= upscale_factor;
    desc.Height *= upscale_factor;
  }
  ...
  desc.Flags = upscale_factor > 1 ? D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS
...
bool D3D12TextureCache::LoadTextureDataFromResidentMemoryImpl(Texture& texture, bool load_base,
  ...
  uint32_t texture_upscale_factor = GetTextureUpscaleFactor(texture_key);
  bool texture_upscaled = texture_upscale_factor > 1;
  ...
  // DBZ3: si la textura es de upscale, en vez de copiar el buffer 1x a la
  // textura, se rellena la textura Nx con la pasada bicubica (capa exterior,
  // sin tocar la memoria guest ni ningun fichero del juego).
  if (texture_upscaled) {
    // La fuente del upscale es SIEMPRE el nivel 0 (imagen 1x con pitch en el
    // scratch buffer); el shader genera cada nivel promediando bloques de ese
    // nivel 0, asi que no dependemos del layout de mips empaquetados del guest.
    // Si esta llamada solo trae mips (sin la base), la cadena ya se genero al
    // cargar el nivel 0.
    bool upscale_ok = true;
    if (level_first == 0) {
      ...
      for (uint32_t level = 0; level <= last_level; ++level) {
        bool level_ok = UpscaleTextureData(
            ...
            copy_buffer_size, uint32_t(host_slice_layout_base.Footprint.RowPitch),
            uint32_t(host_slice_layout_base.Offset));
        if (!level_ok) {
          // Un fallo en un nivel alto (p.ej. sin descriptores libres) no debe
          // invalidar la carga: se conserva lo que ya se haya generado.
          static int level_fail_logged = 0;
          if (level_fail_logged < 6) {
            ++level_fail_logged;
            REXGPU_WARN(
                "dbz3: texture upscale level {} failed ({}x{} mips {}, factor {}) - base kept",
                level, texture_key.GetWidth(), texture_key.GetHeight(), last_level,
        ...
        if (level == 0) {
          ...
          if (first_logged < 6) {
            ++first_logged;
            REXGPU_INFO("dbz3: upscale first texture {}x{} mips={} factor={}", texture_key.GetWidth(),
                        texture_key.GetHeight(), last_level, texture_upscale_factor);
          }
        }
      }
    }
    if (upscale_ok) {
      ++upscaled_texture_count_;
    }
    command_processor_.ReleaseScratchGPUBuffer(copy_buffer, copy_buffer_state);
    return upscale_ok;
  }
...
bool D3D12TextureCache::InitializeTextureUpscale() {
  const ui::d3d12::D3D12Provider& provider = command_processor_.GetD3D12Provider();
  ID3D12Device* device = provider.GetDevice();
  // Parametro 0: constantes; 1: SRV (ByteAddressBuffer 1x); 2: UAV (textura Nx).
  D3D12_ROOT_PARAMETER root_parameters[3];
  root_parameters[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;
  root_parameters[0].Constants.ShaderRegister = 0;
  root_parameters[0].Constants.RegisterSpace = 0;
  root_parameters[0].Constants.Num32BitValues = sizeof(TextureUpscaleConstants) / sizeof(uint32_t);
  root_parameters[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
  D3D12_DESCRIPTOR_RANGE upscale_source_range;
  upscale_source_range.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
  upscale_source_range.NumDescriptors = 1;
  upscale_source_range.BaseShaderRegister = 0;
  upscale_source_range.RegisterSpace = 0;
  upscale_source_range.OffsetInDescriptorsFromTableStart = 0;
  root_parameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  root_parameters[1].DescriptorTable.NumDescriptorRanges = 1;
  root_parameters[1].DescriptorTable.pDescriptorRanges = &upscale_source_range;
  root_parameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
  D3D12_DESCRIPTOR_RANGE upscale_dest_range;
  upscale_dest_range.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
  upscale_dest_range.NumDescriptors = 1;
  upscale_dest_range.BaseShaderRegister = 0;
  upscale_dest_range.RegisterSpace = 0;
  upscale_dest_range.OffsetInDescriptorsFromTableStart = 0;
  root_parameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  root_parameters[2].DescriptorTable.NumDescriptorRanges = 1;
  root_parameters[2].DescriptorTable.pDescriptorRanges = &upscale_dest_range;
  root_parameters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
  D3D12_ROOT_SIGNATURE_DESC root_signature_desc;
  root_signature_desc.NumParameters = UINT(rex::countof(root_parameters));
  root_signature_desc.pParameters = root_parameters;
  ...
  root_signature_desc.pStaticSamplers = nullptr;
  root_signature_desc.Flags = D3D12_ROOT_SIGNATURE_FLAG_NONE;
  *(upscale_root_signature_.ReleaseAndGetAddressOf()) =
      ui::d3d12::util::CreateRootSignature(provider, root_signature_desc);
  if (!upscale_root_signature_) {
    REXGPU_ERROR("D3D12TextureCache: Failed to create the texture upscale root signature");
    return false;
  }
  *(upscale_pipeline_.ReleaseAndGetAddressOf()) = ui::d3d12::util::CreateComputePipeline(
      device, shaders::texture_upscale_cs, sizeof(shaders::texture_upscale_cs),
      upscale_root_signature_.Get());
  if (!upscale_pipeline_) {
    REXGPU_ERROR("D3D12TextureCache: Failed to create the texture upscale pipeline");
    return false;
  }
  REXGPU_INFO("D3D12TextureCache: DBZ3 texture upscale pipeline ready (x{})",
              uint32_t(REXCVAR_GET(dbz3_texture_upscale)));
  return true;
}

bool D3D12TextureCache::UpscaleTextureData(D3D12Texture& texture, ID3D12Resource* copy_buffer,
                                           D3D12_RESOURCE_STATES& copy_buffer_state,
                                           uint32_t level, uint32_t upscale_factor,
                                           uint64_t copy_buffer_size, uint32_t src_pitch,
                                           uint32_t src_offset) {
  if (!upscale_pipeline_) {
    return false;
  }
  ...
  // `src_*` son las dimensiones del NIVEL 0 (lo que hay en el buffer 1x); el
  // shader promedia bloques 2^level para los niveles siguientes.
  uint32_t src_width = key.GetWidth();
  uint32_t src_height = key.GetHeight();
  uint32_t level_src_width = std::max(src_width >> level, uint32_t(1));
  uint32_t level_src_height = std::max(src_height >> level, uint32_t(1));
  uint32_t dst_width = level_src_width * upscale_factor;
  uint32_t dst_height = level_src_height * upscale_factor;
  ID3D12Resource* dest_resource = texture.resource();

  // Fuente: buffer RGBA8 1x. Destino: textura Nx (UAV).
  command_processor_.PushTransitionBarrier(copy_buffer, copy_buffer_state,
                                           D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
  copy_buffer_state = D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
  command_processor_.PushTransitionBarrier(
      dest_resource, texture.SetResourceState(D3D12_RESOURCE_STATE_UNORDERED_ACCESS),
      D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
  command_processor_.SubmitBarriers();

  ui::d3d12::util::DescriptorCpuGpuHandlePair descriptors_allocated[2];
  if (!command_processor_.RequestOneUseSingleViewDescriptors(2, descriptors_allocated)) {
    static int dbz3_up_descfail = 0;
    if (dbz3_up_descfail < 8) {
      ++dbz3_up_descfail;
      REXGPU_ERROR("dbz3: upscale descriptor request FAILED (lvl {}, {}x{})", level,
  ...
  srv_desc.Buffer.NumElements = uint32_t(copy_buffer_size >> 2);
  srv_desc.Buffer.StructureByteStride = 0;
  srv_desc.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_RAW;
  device->CreateShaderResourceView(copy_buffer, &srv_desc, descriptors_allocated[0].first);
  D3D12_UNORDERED_ACCESS_VIEW_DESC uav_desc = {};
  uav_desc.Format = GetDXGIUnormFormat(key);
  uav_desc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
  uav_desc.Texture2D.MipSlice = level;
  uav_desc.Texture2D.PlaneSlice = 0;
  device->CreateUnorderedAccessView(dest_resource, nullptr, &uav_desc,
                                    descriptors_allocated[1].first);

  command_processor_.SetExternalPipeline(upscale_pipeline_.Get());
  command_list.D3DSetComputeRootSignature(upscale_root_signature_.Get());
  command_list.D3DSetComputeRootDescriptorTable(1, descriptors_allocated[0].second);
  command_list.D3DSetComputeRootDescriptorTable(2, descriptors_allocated[1].second);
  TextureUpscaleConstants constants;
  constants.src_width = src_width;
  constants.src_height = src_height;
  constants.dst_width = dst_width;
  constants.dst_height = dst_height;
  constants.src_pitch = src_pitch;
  constants.src_offset = src_offset;
  constants.factor = upscale_factor;
  constants.level = level;
  command_list.D3DSetComputeRoot32BitConstants(0, sizeof(constants) / sizeof(uint32_t), &constants,
                                               0);
  command_list.D3DDispatch((dst_width + 7) >> 3, (dst_height + 7) >> 3, 1);
  command_processor_.SubmitBarriers();
  // La textura queda en estado UNORDERED_ACCESS; el binding de texturas hara la
  // transicion a shader resource cuando se use.
  return true;
}
