...
// DBZ3 HD Collection: packs de texturas (modulo comun a D3D12 y Vulkan).
...
void VulkanTextureCache::BeginSubmission(uint64_t new_submission_index) {
  ...
  // DBZ3: liberar los buffers de subida de packs cuya submission ya termino.
  if (!pending_pack_uploads_.empty()) {
    ...
    auto remove_end = std::remove_if(
        pending_pack_uploads_.begin(), pending_pack_uploads_.end(),
        [this, completed_submission](const PendingPackUpload& upload) {
          if (upload.submission > completed_submission) {
            return false;
          }
          vmaDestroyBuffer(vma_allocator_, upload.buffer, upload.allocation);
          return true;
        });
    pending_pack_uploads_.erase(remove_end, pending_pack_uploads_.end());
  }
...
VkImageView VulkanTextureCache::RequestSwapTexture(uint32_t& width_scaled_out,
  ...
  key = texture->key();
  // DBZ3: recordar la clave del frontbuffer para que los packs no la
  // reemplacen.
...
std::unique_ptr<TextureCache::Texture> VulkanTextureCache::CreateTexture(TextureKey key) {
  ...
  // DBZ3: si la textura esta en un pack, el recurso host se crea con las
  // dimensiones de la imagen del pack (el formato ya lo devuelve
  // GetHostFormatPair como RGBA8). El pack nunca es 3D, cubemap ni scaled
  // resolve.
  uint32_t host_width = key.GetWidth();
  uint32_t host_height = key.GetHeight();
  if (const Dbz3TexturePackEntry* pack_entry = FindPackEntry(key)) {
    host_width = pack_entry->width;
    host_height = pack_entry->height;
  }
  ...
  image_create_info.extent.width = host_width;
  image_create_info.extent.height = host_height;
...
bool VulkanTextureCache::LoadTextureDataFromResidentMemoryImpl(Texture& texture, bool load_base,
  ...
  // DBZ3: si la textura esta en un pack, se sube la imagen del pack (RGBA8 con
  // mips) y se omite la carga estandar. Si la subida falla, se sigue con el
  // camino normal.
  if (IsTexturePackReplaced(texture_key) &&
      UploadPackTextureData(vulkan_texture, texture_key)) {
    return true;
  }
...
// DBZ3 HD Collection: packs de texturas (ver dbz3_texture_pack.h).
...
uint32_t VulkanTextureCache::GetTexturePackFactor(const TextureKey& key) const {
  ...
  do {
    ...
    // Mismas restricciones estructurales que en D3D12: solo 2D simples con
    ...
    if (entry->width % width != 0 || entry->height % height != 0) {
      ...
      if (mismatch_logged < 12) {
        ++mismatch_logged;
        REXGPU_WARN("dbz3: pack '{}' {}x{} no es multiplo de {}x{} (ignorado)", entry->pack_name,
    ...
    if (fx != fy || fx < 1 || fx > 4) {
      ...
      if (factor_logged < 12) {
        ++factor_logged;
        REXGPU_WARN("dbz3: pack '{}' factor invalido ({}x{} vs {}x{}; max x4)", entry->pack_name,
                    entry->width, entry->height, width, height);
      }
      entry = nullptr;
      break;
...
const Dbz3TexturePackEntry* VulkanTextureCache::FindPackEntry(const TextureKey& key) const {
  GetTexturePackFactor(key);
  auto it = pack_entry_cache_.find(key);
  return it != pack_entry_cache_.end() ? it->second : nullptr;
}

bool VulkanTextureCache::UploadPackTextureData(VulkanTexture& texture, const TextureKey& key) {
  ...
  if (vmaCreateBuffer(vma_allocator_, &buffer_create_info, &allocation_create_info, &staging_buffer,
                      &staging_allocation, &staging_allocation_info) != VK_SUCCESS) {
    REXGPU_WARN("dbz3: pack '{}': no se pudo crear el buffer de subida", entry->pack_name);
    return false;
  }
  std::memcpy(staging_allocation_info.pMappedData, buffer.data(), buffer.size());
  vmaFlushAllocation(vma_allocator_, staging_allocation, 0, buffer.size());

  DeferredCommandBuffer& command_buffer = command_processor_.deferred_command_buffer();

  // El buffer se escribio desde el host: barrera HOST -> TRANSFER.
  command_processor_.PushBufferMemoryBarrier(
      staging_buffer, 0, VK_WHOLE_SIZE, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
      VK_ACCESS_HOST_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT);

  // La imagen pasa a destino de transferencia (misma transicion que usa la
  // carga estandar); RequestTextures la pasara luego a sampled.
  texture.MarkAsUsed();
  VulkanTexture::Usage texture_old_usage =
  ...
  if (texture_old_usage != VulkanTexture::Usage::kTransferDestination) {
    ...
    GetTextureUsageMasks(VulkanTexture::Usage::kTransferDestination, texture_dst_stage_mask,
                         texture_dst_access_mask, texture_new_layout);
    command_processor_.PushImageMemoryBarrier(
        texture.image(), ui::vulkan::util::InitializeSubresourceRange(), texture_src_stage_mask,
        texture_dst_stage_mask, texture_src_access_mask, texture_dst_access_mask, texture_old_layout,
        texture_new_layout);
  }
  command_processor_.SubmitBarriers(true);
  VkBufferImageCopy* copy_regions = command_buffer.CmdCopyBufferToImageEmplace(
      staging_buffer, texture.image(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, levels);
  for (uint32_t l = 0; l < levels; ++l) {
    const Dbz3PackLevel& layout = layouts[l];
    VkBufferImageCopy& copy_region = copy_regions[l];
    copy_region.bufferOffset = layout.offset;
    copy_region.bufferRowLength = layout.row_pitch / 4;
    copy_region.bufferImageHeight = 0;
    copy_region.imageSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    copy_region.imageSubresource.mipLevel = l;
    copy_region.imageSubresource.baseArrayLayer = 0;
    copy_region.imageSubresource.layerCount = 1;
    copy_region.imageOffset.x = 0;
    copy_region.imageOffset.y = 0;
    copy_region.imageOffset.z = 0;
    copy_region.imageExtent.width = layout.width;
    copy_region.imageExtent.height = layout.height;
  ...
  if (upload_logged < 48) {
    ...
    REXGPU_INFO("dbz3: pack '{}' subido {}x{} ({} niveles, {} B)", entry->pack_name, pack_width,
                pack_height, levels, buffer.size());
  }
  return true;
}
...
const VulkanTextureCache::HostFormatPair& VulkanTextureCache::GetHostFormatPair(
  ...
  // DBZ3: una textura reemplazada por un pack se sube como RGBA8 con swizzle
  // identidad, sea cual sea el formato guest original. Se reutiliza el par host
  // de k_8_8_8_8 (R8G8B8A8 UNORM/SNORM, swizzle RGBA).
  if (IsTexturePackReplaced(key)) {
    return kBestHostFormats[uint32_t(xenos::TextureFormat::k_8_8_8_8)];
  }
