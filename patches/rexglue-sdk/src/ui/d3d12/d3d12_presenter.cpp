...
Presenter::PaintResult D3D12Presenter::PaintAndPresentImpl(bool execute_ui_drawers) {
  ...
  // Startup timing diagnostic: log the FIRST successful present so the
  // black-screen window (window created -> first painted frame) can be
  // measured from the game log on machines where the launcher is slow.
  static bool dbz3_first_present_logged = false;
  if (!dbz3_first_present_logged && SUCCEEDED(present_result)) {
    dbz3_first_present_logged = true;
    static const std::chrono::steady_clock::time_point boot_clock =
        std::chrono::steady_clock::now();
    const auto elapsed_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now() - boot_clock)
            .count();
    REXLOG_INFO("dbz3: first present OK (device/swapchain init + first paint took {} ms)",
                elapsed_ms);
  }
