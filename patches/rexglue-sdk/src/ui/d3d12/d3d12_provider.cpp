...
D3D12Provider::~D3D12Provider() {
  ...
  if (adapter_ != nullptr) {
    // DBZ3: referencia al adaptador conservada para la consulta de VRAM.
    adapter_->Release();
  }
...
bool D3D12Provider::Initialize() {
  ...
  // DBZ3: se conserva el adaptador (antes se soltaba tras crear el dispositivo)
  // para poder consultar la VRAM con IDXGIAdapter3::QueryVideoMemoryInfo.
  adapter_ = adapter;
  ...
  // DBZ3: el adaptador queda vivo en adapter_ (ver arriba); no se suelta aqui.
