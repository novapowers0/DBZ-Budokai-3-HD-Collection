...
// Keep FIFO as the safe default on composited Linux desktops. Immediate and
// mailbox can create an unbounded present loop, which is especially expensive
// when MangoHud or Steam hooks every vkQueuePresentKHR call.
REXCVAR_DEFINE_BOOL(vulkan_allow_present_mode_immediate, false, "UI/Vulkan",
...
// to avoid a duplicate symbol when both presenters are built; here we only
// declare it. It does not alter guest vblank or game timing.
...
VkSwapchainKHR VulkanPresenter::PaintContext::CreateSwapchainForVulkanSurface(
  ...
  if (!host_present_cap && REXCVAR_GET(vulkan_allow_present_mode_immediate) &&
  ...
  } else if (!host_present_cap && REXCVAR_GET(vulkan_allow_present_mode_mailbox) &&
