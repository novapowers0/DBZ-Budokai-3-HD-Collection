...
void Presenter::WaitForUITickFromUIThread() {
  ...
  while (true) {
    ...
    // Safety net: never let the UI thread block indefinitely waiting for a
    // vblank tick. If the DXGI output stops signalling (lost/stale monitor,
    // WaitForVBlank stuck, display-mode change), a bare wait here would stall
    // the paint, make the window "Not Responding" and starve Alt+F4 / the
    // window-close event (observed with the launcher going black). Bounding the
    // wait keeps a ~20 FPS fallback paint cadence so the UI thread always
