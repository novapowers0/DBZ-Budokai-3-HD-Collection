...
std::filesystem::path NextSequentialLogPath(const std::filesystem::path& logs_dir,
  ...
  std::error_code ec;
  std::vector<std::pair<int, std::filesystem::path>> previous_runs;
  for (const auto& entry : std::filesystem::directory_iterator(logs_dir, ec)) {
    ...
    if (stem.starts_with(prefix)) {
      ...
      if (parse_ec == std::errc() && ptr == num_str.data() + num_str.size()) {
        ...
        previous_runs.emplace_back(num, entry.path());
      }
    }
  }

  // Each run gets a fresh sequential name, so the rotating sink's own cap never
  // applies across runs and the logs folder would grow forever. Honor
  // `log_max_files` here by dropping the oldest runs before creating the new one.
  int keep = REXCVAR_GET(log_max_files);
  if (keep < 1) {
    keep = 1;
  }
  if (static_cast<int>(previous_runs.size()) >= keep) {
    std::sort(previous_runs.begin(), previous_runs.end(),
              [](const auto& a, const auto& b) { return a.first < b.first; });
    const size_t to_remove = previous_runs.size() - static_cast<size_t>(keep) + 1;
    for (size_t i = 0; i < to_remove; ++i) {
      std::error_code remove_ec;
      std::filesystem::remove(previous_runs[i].second, remove_ec);
    }
  }
