...
class D3D12Provider : public GraphicsProvider {
 public:
  ...
  // DBZ3: adaptador elegido (se conserva una referencia viva). El dispositivo no
  // implementa IDXGIDevice (es un objeto propio), asi que la VRAM se consulta por
  // el adaptador: IDXGIAdapter3::QueryVideoMemoryInfo.
  IDXGIAdapter* GetAdapter() const { return adapter_; }
 ...
 private:
  ...
  // DBZ3: adaptador elegido, conservado para la telemetria/guardia de VRAM (ver
  // GetAdapter()).
