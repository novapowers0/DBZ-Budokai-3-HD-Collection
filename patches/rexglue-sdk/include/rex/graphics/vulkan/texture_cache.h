...
class VulkanTextureCache final : public TextureCache {
 public:
  ...
  // DBZ3 HD Collection: "packs" de texturas al estilo PCSX2 (ver
  // dbz3_texture_pack.h). Un pack reemplaza la textura host por la imagen del
  // pack (RGBA8, a la resolucion del pack). El factor (0 = sin pack, 1..4 =
  // reemplazo) se deduce de W_pack / W_guest. Cacheado por textura.
 ...
 private:
  ...
  // DBZ3 HD Collection: packs de texturas.
  // Linealiza el bitmap guest (mismo dato que el volcado dev) para calcular su
  // hash. Devuelve false si la textura no es apta (no 2D, tiled raro, etc.).
  ...
  // si no se pudo (en ese caso se sigue con la carga normal).
  bool UploadPackTextureData(VulkanTexture& texture, const TextureKey& key);

  // Cache del factor/entrada del pack por textura. El resultado (incluido "no
  // hay pack") se memoriza para que la respuesta sea estable entre la creacion
  // del recurso y las subidas posteriores.
  ...
  // submission).
  struct PendingPackUpload {
    uint64_t submission;
    VkBuffer buffer;
    VmaAllocation allocation;
  };
  std::vector<PendingPackUpload> pending_pack_uploads_;
  // Clave de la textura de origen del presentador (la ultima pedida por
  // RequestSwapTexture); un pack nunca debe reemplazarla.
  TextureKey swap_texture_key_;
  bool swap_texture_key_valid_ = false;
